...
const EmployeeDeleteModal = ({assetId, waitTime = 3, onClose, ...props}) => {
...
const VehicleDeleteModal = ({assetId, waitTime = 3, onClose, ...props}) => {

    const [ count, setCount ] = useState(waitTime)
    const { dispatcher } = useVehicleContext()
        ...
        <Modal>
            <div className="bg-white w-[450px] p-4 relative" {...props}>
                <ModalCloseButton
                    onClick={onClose}
                />
                <h3 className="font-medium text-2xl">Are your sure to delete this Vehicle</h3>
                {count > 0 &&
                <div className="m-auto rounded-full border-2 border-red-500 w-8 h-8 text-lg text-red-500 flex items-center justify-center mt-2">
                    <span>
                        {count}
...
export { EmployeeDeleteModal, VehicleDeleteModal }